...
const findItem = require('./util/find-item');
...
function buildGenerator(sources) {
	return function generator(result = {tags: []}) {
		...
		for (const {name, items} of sources) {
			...
			// If the property is set, match it to an existing item
			if (result[name]) {
				item = findItem(items, result[name]);

			// Otherwise get a random one
			} else {
				item = randomItem(enforceFilters(items, result.tags));
			}
